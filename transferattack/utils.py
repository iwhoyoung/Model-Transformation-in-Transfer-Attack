...
cnn_model_paper = ['resnet18', 'resnet50', 'resnet101', 'resnext50_32x4d', 'densenet121', "vgg19", "inception_v3"]
vit_model_paper = ['vit_base_patch16_224', 'pit_b_224',
                   'visformer_small', 'swin_tiny_patch4_window7_224']
mamba_model_paper = ['mamba_vision_T', 'mamba_vision_S', 'mamba_vision_L']
...
def load_pretrained_model(cnn_model=[], vit_model=[]):
    ...
    for model_name in vit_model:
        if model_name == "vit_small_patch16_224":
            model_path = os.environ.get("VIT_SMALL_CHECKPOINT")
            if not model_path:
                print("=> Skipping vit_small_patch16_224 because VIT_SMALL_CHECKPOINT is not set")
                continue
