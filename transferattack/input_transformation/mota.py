...
class BlockShuffle(nn.Module):
    ...
    def _random_partition(self, length):
        block_count = min(self.num_block, length)
        if block_count <= 1:
            return (length,)
        cuts = sorted(random.sample(range(1, length), block_count - 1))
        points = [0, *cuts, length]
        return tuple(points[i + 1] - points[i] for i in range(block_count))
